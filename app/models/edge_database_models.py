...
class AsignacionConductorBusLocal(Base):
    ...
    tipo_asignacion = Column(String, nullable=True)
...
class TelemetryLocal(Base):
    """
    Modelo para almacenar los datos de telemetría del Jetson Nano localmente.
    Estos datos serán sincronizados con la tabla 'jetson_nanos' en la nube.
    """
    __tablename__ = 'telemetry_local'
    id = Column(UUIDType, primary_key=True, default=uuid.uuid4, unique=True, nullable=False)
    id_hardware_jetson = Column(String, nullable=False) # ID del hardware Jetson
    ...
    disk_usage_percent = Column(Numeric) # Added for consistency with gather_system_metrics
    temperatura_celsius = Column(Numeric)
    synced_to_cloud = Column(Boolean, default=False, nullable=False)
    sent_to_cloud_at = Column(DateTime)

    def __repr__(self):
        return (f"<TelemetryLocal(id='{self.id}', jetson_id='{self.id_hardware_jetson}', "
                f"time='{self.timestamp_telemetry}', synced='{self.synced_to_cloud}')>")